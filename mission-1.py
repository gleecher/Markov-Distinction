...
class MarkovArtist:
    ...
    def calc_new_red(self):
        ...
        """        
        self.redval = np.random.choice(range(0,255), \
                                       p = self.transition_matrix[self.redval])
    ...
    def calc_new_green(self):
        ...
        """          
        self.greenval = \
            np.random.choice(range(0,255), \
            p = self.transition_matrix[self.greenval])
        
    def calc_new_blue(self):
        """
        Decides what the next value for blue should be based on the current val
        """          
        self.blueval = np.random.choice(range(0,255), \
                                    p = self.transition_matrix[self.blueval])
    ...
    def draw_image(self, width, height, offset):
        ...
        selected through a markov process
            ...
            width (int) the width of the final image
            height (int) the height of the final image
            offset (int) the length and width of the squares
        """          
        ...
        for i in range(offset, height - offset, offset): 
            ...
            for j in range(offset, width - offset, offset):
                ...
                #Updates the colors based on their current values
                self.calc_new_red()
                self.calc_new_green()
                self.calc_new_blue()
                ...
                #draws the rectangle in the spot based on the two for loops and
                #the current color
